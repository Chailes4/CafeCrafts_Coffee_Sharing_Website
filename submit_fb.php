...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) { // If successful
        ...
        header("Location: fb.html?status=success&message=Thank you for your feedback!");
        exit; // Exit after header
    } else { // If there's an error
        // Redirect to feedback.html with an error message
        header("Location: fb.html?status=error&message=An error occurred. Please try again.");
        exit; // Exit after header
    }
